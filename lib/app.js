...
import { createEmptyCapture, assignSession, APP_VERSION } from "./schema.js";
...
import { storageErrorKey } from "./errors.js";
import { audioFilename } from "./audio-filename.js";
...
/** @see lib/errors.js for the pure classification logic (kept there so it's unit-testable). */
...
async function renderCaptureCard(capture) {
  ...
  const hasAudio = Boolean(capture.audio_blob_ref);
  ...
  if (hasAudio) wireAudioPlayer(card, capture);
...
/** @see lib/audio-filename.js for the pure logic (kept there so it's unit-testable). */
 ...
 */
function wireAudioPlayer(card, capture) {
  ...
  const downloadBtn = container.querySelector("[data-action='download-audio']");
  const iconPlay = playBtn.querySelector(".icon-play");
  const iconPause = playBtn.querySelector(".icon-pause");
  const seek = container.querySelector(".audio-seek");
  const timeLabel = container.querySelector(".audio-time");

  // The audio blob is NOT fetched here — only metadata (transcript, times,
  // status) loads when Review renders. Reading and decoding every capture's
  // audio on every render doesn't scale once there are months of nights in
  // the database. The actual blob is fetched lazily, once, on first play or
  // download click, then cached in this closure for the rest of the session.
  let cachedBlob = null;
  ...
  async function ensureBlobLoaded() {
    ...
    if (!loadPromise) {
      loadPromise = db.getAudioBlob(capture.audio_blob_ref).then((blob) => {
  ...
  playBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      playBtn.disabled = false;
      showToast(t("error.audioLoadFailed"));
    }
  });
  ...
  downloadBtn?.addEventListener("click", async () => {
    try {
      ...
      const blob = await ensureBlobLoaded();
      downloadBlob(blob, audioFilename(capture));
    } catch {
      showToast(t("error.audioLoadFailed"));
    } finally {
...
async function renderSettings() {
  $("#app-version").textContent = `Echo v${APP_VERSION}`;
  ...
  $("#backup-create-btn").onclick = () => createBackupFile();
  $("#backup-restore-btn").onclick = () => $("#backup-file-input").click();
...
async function createBackupFile() {
  ...
  try {
    ...
    downloadBlob(blob, `echo-backup-${new Date().toISOString().slice(0, 10)}.json`);
    showToast(t("settings.backup.createSuccess").replace("{n}", payload.capture_count));
  } catch {
    await showAlert(t("settings.backup.createFailed"));
  } finally {
...
async function restoreBackupFile(file) {
  ...
  try {
    ...
    await showAlert(t("settings.backup.restoreFailed"));
  } finally {
